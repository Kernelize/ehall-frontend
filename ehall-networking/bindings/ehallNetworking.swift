...
#if canImport(ehallNetworkingFFI)
import ehallNetworkingFFI
#endif
...
public func rustTokioTest()  -> String {
    return try!  FfiConverterString.lift(
        try! rustCall() {
    uniffi_ehall_networking_fn_func_rust_tokio_test($0)
}
    )
}
...
private var initializationResult: InitializationResult {
    ...
    if (uniffi_ehall_networking_checksum_func_rust_add() != 49749) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_ehall_networking_checksum_func_rust_greeting() != 38795) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_ehall_networking_checksum_func_rust_tokio_test() != 28974) {
        return InitializationResult.apiChecksumMismatch
    }
